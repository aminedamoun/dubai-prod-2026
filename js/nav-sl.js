/**
 * Dubai Prod — Shared Mega Menu Nav (SLOVENIAN)
 * Slovenian version of the navbar for /sl/ pages.
 */
(function () {
  ...
  /* ── Service items (SLOVENIAN) ── */
  var COLS = [
    {
      title: 'Kreativa',
      items: [
        { label: 'CGI & 3D',            desc: 'Fotorealistični renderi & animacije',      icon: 'box',     href: 'sl/cgi-3d.html' },
        { label: 'Video produkcija',     desc: 'Filmska produkcija za blagovne znamke',    icon: 'film',    href: 'sl/video-produkcija.html' },
        { label: 'Blagovna znamka',      desc: 'Identiteta & vizualna smer',               icon: 'palette', href: 'sl/blagovna-znamka.html' }
      ]
    },
    {
      title: 'Rast',
      items: [
        { label: 'PR & Mediji',          desc: 'Medijska pokritost za avtoriteto',         icon: 'newspaper', href: 'sl/odnosi-z-javnostmi.html' },
        { label: 'Družbena omrežja',     desc: 'Vsebine za večjo angažiranost',            icon: 'share',     href: 'sl/druzbena-omrezja.html' },
        { label: 'Oglaševanje',          desc: 'Plačljivi mediji, ki konvertirajo',        icon: 'target',    href: 'sl/oglasevanje.html' }
      ]
    },
    {
      title: 'Digitalno',
      items: [
        { label: 'SEO',                  desc: 'Organska rast & dominacija iskanja',       icon: 'search',  href: 'sl/seo.html' },
        { label: 'GEO & AI iskanje',     desc: 'Najdite se na ChatGPT & AI',              icon: 'star',    href: 'sl/geo-optimizacija.html' },
        { label: 'Spletne strani',       desc: 'Hitre, lepe digitalne izkušnje',           icon: 'globe',   href: 'sl/spletne-strani.html' },
        { label: 'AI avtomatizacija',    desc: 'Marketing na avtopilotu',                  icon: 'zap',     href: 'sl/ai-avtomatizacija.html' }
      ]
    }
  ];
  ...
  var inSl = path.indexOf('/sl/') !== -1 || path.endsWith('/sl');
  var inPortfolio = path.indexOf('/portfolio/') !== -1;
  var depth = path.split('/').length - 2;
  var prefix = '';
  if (inPortfolio && inSl) {
    prefix = '../../';
  } else if (inSl || inPortfolio) {
    prefix = '../';
  }
  ...
  var langSwitch = (function() {
    var enHref = document.documentElement.getAttribute('data-en-href') || '/';
    return '<a href="' + enHref + '" style="font-size:11px;font-weight:700;letter-spacing:0.1em;color:rgba(255,255,255,0.5);text-decoration:none;padding:6px 10px;border:1px solid rgba(255,255,255,0.15);border-radius:4px;margin-right:12px;transition:all 0.2s;" onmouseenter="this.style.color=\'#FF6A00\';this.style.borderColor=\'rgba(255,106,0,0.5)\'" onmouseleave="this.style.color=\'rgba(255,255,255,0.5)\';this.style.borderColor=\'rgba(255,255,255,0.15)\'">EN</a>' +
           '<span style="font-size:11px;font-weight:700;letter-spacing:0.1em;color:#FF6A00;padding:6px 10px;border:1px solid rgba(255,106,0,0.5);border-radius:4px;margin-right:16px;">SL</span>';
  })();

  /* ── Full nav HTML ── */
  var NAV_HTML =
    '<a href="' + prefix + 'sl/index.html" class="mm-logo">' +
    ...
    '<ul class="mm-list" id="mm-list">' +
      '<li class="mm-item"><a href="' + prefix + 'sl/index.html" class="mm-btn">Domov</a></li>' +
      '<li class="mm-item mm-has-sub" id="mm-services-item">' +
        ...
        '</div>' +
      '</li>' +
      '<li class="mm-item"><a href="' + prefix + 'sl/o-nas.html" class="mm-btn">O nas</a></li>' +
      '<li class="mm-item"><a href="' + prefix + 'sl/portfolio.html" class="mm-btn">Portfolio</a></li>' +
      '<li class="mm-item"><a href="' + prefix + 'sl/blog.html" class="mm-btn">Blog</a></li>' +
      '<li class="mm-item"><a href="' + prefix + 'sl/kontakt.html" class="mm-btn">Kontakt</a></li>' +
    '</ul>' +

    /* Hamburger (mobile) */
    '<button class="mm-hamburger" id="mm-hamburger" aria-label="Meni">' + ICONS.menu + '</button>' +

    /* Language switch + Connect */
      ...
      '<div id="connect-form-panel" style="position:absolute;top:calc(100% + 12px);right:0;width:360px;max-height:0;overflow:hidden;transition:max-height 0.55s cubic-bezier(0.4,0,0.2,1),opacity 0.4s ease;opacity:0;z-index:200;">' +
        '<div style="background:rgba(10,10,10,0.95);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);border:1px solid rgba(255,106,0,0.4);box-shadow:0 24px 60px rgba(0,0,0,0.7);padding:28px 28px 24px;">' +
          '<div style="margin-bottom:20px;">' +
            '<p style="font-size:9px;letter-spacing:0.35em;color:#ff6a00;font-weight:700;text-transform:uppercase;margin-bottom:6px;">// Kontaktirajte nas</p>' +
            '<h3 style="font-family:Inter,sans-serif;font-size:18px;font-weight:800;color:#fff;">SODELUJMO SKUPAJ</h3>' +
          '</div>' +
          '<form id="connect-contact-form" onsubmit="handleConnectSubmit(event)" style="display:flex;flex-direction:column;gap:14px;">' +
            '<div><label style="font-size:9px;letter-spacing:0.25em;color:rgba(255,255,255,0.4);text-transform:uppercase;font-weight:600;display:block;margin-bottom:5px;">E-pošta *</label>' +
            '<input type="email" name="email" required placeholder="vaš@email.com" style="width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);padding:10px 14px;color:#fff;font-size:13px;font-family:Inter,sans-serif;outline:none;" onfocus="this.style.borderColor=\'rgba(255,106,0,0.6)\'" onblur="this.style.borderColor=\'rgba(255,255,255,0.1)\'"></div>' +
            '<div><label style="font-size:9px;letter-spacing:0.25em;color:rgba(255,255,255,0.4);text-transform:uppercase;font-weight:600;display:block;margin-bottom:5px;">Telefon</label>' +
            '<input type="tel" name="phone" placeholder="+386 40 000 000" style="width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);padding:10px 14px;color:#fff;font-size:13px;font-family:Inter,sans-serif;outline:none;" onfocus="this.style.borderColor=\'rgba(255,106,0,0.6)\'" onblur="this.style.borderColor=\'rgba(255,255,255,0.1)\'"></div>' +
            '<button type="submit" style="width:100%;background:linear-gradient(90deg,#ff6a00,#ff9500);color:#000;font-size:11px;font-weight:800;letter-spacing:0.3em;text-transform:uppercase;padding:13px;border:none;cursor:pointer;font-family:Inter,sans-serif;">POŠLJI SPOROČILO &rarr;</button>' +
          '</form>' +
          '<div id="connect-success" style="display:none;text-align:center;padding:30px 0;">' +
            ...
            '<p style="color:#ff6a00;font-size:11px;letter-spacing:0.3em;font-weight:700;text-transform:uppercase;">Sporočilo poslano!</p>' +
          '</div>' +
        '</div>' +
      '</div>' +
    '</div>';
    ...
    '<div id="mm-mobile-drawer" class="mm-mobile-drawer">' +
      '<div class="mm-mobile-inner">' +
        '<div style="display:flex;gap:8px;margin-bottom:16px;padding-bottom:16px;border-bottom:1px solid rgba(255,255,255,0.06);">' +
          '<a href="' + (document.documentElement.getAttribute('data-en-href') || '/') + '" style="padding:8px 16px;font-size:12px;font-weight:700;color:rgba(255,255,255,0.5);border:1px solid rgba(255,255,255,0.15);border-radius:4px;text-decoration:none;">EN</a>' +
          '<span style="padding:8px 16px;font-size:12px;font-weight:700;color:#FF6A00;border:1px solid rgba(255,106,0,0.5);border-radius:4px;">SL</span>' +
        '</div>' +
        '<a href="' + prefix + 'sl/index.html" class="mm-mobile-link">Domov</a>' +
        '<div class="mm-mobile-group">' +
          '<button class="mm-mobile-link mm-mobile-group-btn" id="mm-mobile-svc-btn">Storitve <span id="mm-mobile-svc-arrow" style="transition:transform 0.3s;display:inline-block;">▾</span></button>' +
          ...
          '</div>' +
        '</div>' +
        '<a href="' + prefix + 'sl/o-nas.html" class="mm-mobile-link">O nas</a>' +
        '<a href="' + prefix + 'sl/portfolio.html" class="mm-mobile-link">Portfolio</a>' +
        '<a href="' + prefix + 'sl/blog.html" class="mm-mobile-link">Blog</a>' +
        '<a href="' + prefix + 'sl/kontakt.html" class="mm-mobile-link">Kontakt</a>' +
        '<a href="https://wa.me/971543333587" class="mm-mobile-cta">Začnite projekt →</a>' +
      '</div>' +
    '</div>';
